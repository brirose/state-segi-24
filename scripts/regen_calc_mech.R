...
rgn_hist_long_prep <- rgn_hist %>% 
  ...
  left_join(rgn_groves_lookup, by = join_by(dist_id))
# %>% # connect with disturbance attributes
#   select(-dist_id)
...
##what dist class appears for same year?
ro = rgn_order %>%
  filter(recent_dist_yr==prior1_dist_yr)
unique(ro$recent_rgn_dist_class)
unique(ro$prior1_rgn_dist_class)
######################################################
######################################################
######################################################
######################################################

###think i also need to fix mech under later?

#consolidate for disturbances that occcur in same year
mech_rgn_order_fix = rgn_order %>%
  # filter(recent_dist_yr==prior1_dist_yr) %>%
  mutate(recent_rgn_dist_class = case_when(
    ...
    recent_dist_yr==prior1_dist_yr &
      ...
      if_any(c(recent_rgn_dist_class,prior1_rgn_dist_class), ~ . != "mech_under") & 
      if_any(c(recent_rgn_dist_class,prior1_rgn_dist_class), ~ . == "unburned") ~ "unburned",
    recent_dist_yr==prior1_dist_yr &
      if_any(c(recent_rgn_dist_class,prior1_rgn_dist_class), ~ . != "regen_ok") & 
      if_any(c(recent_rgn_dist_class,prior1_rgn_dist_class), ~ . != "unburned") & 
      if_any(c(recent_rgn_dist_class,prior1_rgn_dist_class), ~ . == "mech_under") ~ "mech_under",
...
mech_rgn_tallies = mech_rgn_order_fix %>%
  mutate(TRUES=1) %>% 
  pivot_wider(names_from = recent_rgn_dist_class, values_from = TRUES, values_fill = 0)

mech_rgn_tallies_sum = mech_rgn_tallies %>%
  group_by(resist_id) %>%
  summarise(across(c(mod_risk_to_rgn:high_risk_to_rgn), sum)) %>%
  mutate(area_ha = as.numeric(st_area(.))*0.0001)
mech_rgn_tallies_sum

#check for duplicates again 
n.occur = data.frame(table(mech_rgn_tallies_sum$resist_id))
n.occur[n.occur$Freq > 1,]
n.occur2 = n.occur %>%
  filter(Freq > 1)
nrow(n.occur2)

mech_all_rgn_order_lookup = mech_rgn_tallies_sum %>%
  st_drop_geometry()
...
###############################
##Regen totals
mech_rgn_cls = mech_rgn_tallies_sum %>%
  st_make_valid() %>%
  mutate(regen_status =
           case_when(
             ...
             if_all(c(mod_risk_to_rgn,high_risk_to_rgn,regen_ok,mech_under), ~. == 0) & 
               unburned > 0 ~ "Unburned",
             if_all(c(mod_risk_to_rgn,high_risk_to_rgn,regen_ok), ~. == 0) & 
               (mech_under > 0 | unburned >= 0) ~ "Unburned with mechanical",
             .default = "oopsy")) %>%
  # select(regen_status,area_ha) %>%
  group_by(regen_status) %>%
  summarise(area_ha = sum(area_ha))
head(mech_rgn_cls)
sum(mech_rgn_cls$area_ha)

#get shapfile of the area totally unburned and untreated
mech_no_dist <- st_difference(groves, st_union(mech_rgn_cls)) %>%
  st_as_sf() %>%
  st_make_valid() %>%
...
sum(mech_no_dist$area_ha)
sum(mech_rgn_cls$area_ha) + sum(mech_no_dist$area_ha)

mech_rgn_polys_all_v1 = mech_rgn_cls %>%
  rbind(no_dist) %>%
  group_by(regen_status) %>%
  summarise(area_ha = sum(area_ha)) %>%
  mutate(dist_id = 1:length(regen_status))
mech_rgn_polys_all_v1
sum(mech_rgn_polys_all_v1$area_ha)
...
mech_rgn_polys_all_lookup <- mech_rgn_polys_all_v1 %>% 
  st_drop_geometry()

##re-intersect to resolve few polys that did not "flatten" and overlap
mech_rgn_polys_all_2nd_int <- st_intersection(mech_rgn_polys_all_v1) %>% 
  mutate(resist_id = 1:length(origins)) %>% 
  # select(resist_id, origins) %>% 
  st_collection_extract(type = c("POLYGON"), warn = FALSE) %>% #keep only polys
  #recombine multipolygons
  summarise(.by = c(resist_id, origins), geometry = st_combine(geometry)) %>%
  mutate(area_ha = as.numeric(st_area(.))*0.0001)
head(mech_rgn_polys_all_2nd_int)

mech_rgn_polys_all_2nd_int_long <- mech_rgn_polys_all_2nd_int %>% 
  # st_drop_geometry() %>%  # attributes only
  unnest(origins) %>%  # make long dataframe
  rename(dist_id = origins) %>% 
  left_join(mech_rgn_polys_all_lookup, by = join_by(dist_id)) 
# %>% # connect with disturbance attributes
#   select(-dist_id) #%>%
mech_rgn_polys_all_2nd_int_long

##Give ranks to the regen status types so that where there is conflict keep min of those when grouped by resist_id
mech_rgn_polys_all = mech_rgn_polys_all_2nd_int_long %>%
  ...
  mutate(rank = case_when(regen_status == "High risk of inadequate postfire regeneration" ~ 1,
                          ...
                          regen_status == "Unburned with mechanical" ~ 4,
                          regen_status == "Unburned" ~ 5
  )) %>%
  mutate(area_ha = as.numeric(st_area(.))*0.0001) %>%
  group_by(resist_id) %>%
  slice_min(rank) %>%
  ...
  summarise(area_ha = sum(area_ha))
mech_rgn_polys_all
sum(mech_rgn_polys_all$area_ha)
