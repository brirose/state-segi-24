...
treatment <- st_read(here("data/spatial_data/outputs/SEGI_Trtdata/SEGI_Trtdata_04feb25.shp")) %>% 
...
fire_clean <- fire %>% 
  ...
  mutate(
    dist_type = case_when(
      burnsev == 1 ~ "undetected_change", ##CHECK WITH KRISTEN ON THIS
...
```{r classify}
#Add time frame (back 10 years)
#think on ways to actually make this more complex (treatment time + amount in timing)
...
what do these mean?
want amount of area for each of these;make table and shapefiles
...
ggsave(here("outputs/annualtrt_activity_split.jpg"), width = 4.2, height = 5)
```
...
ggsave(here("outputs/annualfire_activity.jpg"), width = 4.2, height = 5)
```
...
ggsave(here("outputs/annualfire_activity_split.jpg"), width = 4.2, height = 5)
```
...
# double ccheck this includes fire (low and moderate)
