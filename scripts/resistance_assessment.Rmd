...
total_groveHa <- sum(as.numeric(st_area(groves_buffered)*0.0001))
...
resist_footprint <- resist_footprint %>% 
 mutate(
    Hectares = round(Hectares, 0),
    perc_area = Hectares/total_grovebufferHa*100
  )

ggplot(resist_footprint, aes(factor(resist_class, 
...
#current mean annual treatment:
  geom_hline(yintercept=mean_annual_trt, linetype='dashed', col = 'black', linewidth = 0.75) +
