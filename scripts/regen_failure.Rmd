...
high_sev <- as.polygons(sev_rdnbr) %>% 
  subset(rdnbr >= 801, NSE = T) %>%  #select  rdnbr 801-1100 mod risk and >1101 high risk
  aggregate()

writeVector(high_sev, here("data/spatial_data/outputs/highsev_debug.shp"), overwrite = T)
```
...
writeVector(disbursal_area, here("data/spatial_data/outputs/disbursal_debug.shp"), overwrite = T)
