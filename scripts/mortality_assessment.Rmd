...
castle_meyer <- read_excel(here("data/plot_data/meyer/CastleFireGroveData_2023_combined.xlsx"), sheet = "Monarchs") %>% 
  clean_names() %>% 
  mutate(source = "Meyer", survey_fire = "post_castle", #include relevant info
  ...
  rename(date_surveyed = date,
         plot = plot_no_nearby) %>% 
  select(id, grove, date_surveyed, easting, northing, postfire_status, prefire_status, source, survey_fire, gps_offset_m, dbh_cm, gps_offset_deg, plot)
...
knp_meyer <- read_excel(here("data/plot_data/meyer/PostfireSequoiaGroveData_2024_FreemanCrk&REMO.v2.reduced.xlsx"), sheet = "Monarchs") %>% 
  clean_names() %>% 
  filter(removed == "no") %>% 
  mutate(source = "Meyer", survey_fire = "knp",
         id = paste(grove, plot_no_nearby, monarch_no, sep = "")) %>% 
...
windy_meyer <- read_excel(here("data/plot_data/meyer/WindyFireSequoiaGroveData.reduced.v2.xlsx"), sheet = "Monarchs") %>% 
  clean_names() %>% 
  mutate(source = "Meyer", survey_fire = "post_windy",
...
monarchs_severity <- monarchs_severity %>% 
  ...
  mutate(in_perim = case_when(!is.na(burnsev) ~ "yes",
                              TRUE ~ "no")) %>% 
  select(tree_id, grove, postfire_status, prefire_status, date_surveyed, survey_fire, fire_id, fire_yr, burnsev, source, in_perim, plot)
...
```{r}
#meyers breakdown
meyers_summary <- monarchs_severity %>% 
  st_drop_geometry() %>% 
  summarise(.by = c(survey_fire, fire_id, grove, plot), count = n())
...
severity_annual_wider <- severity_annual
  pivot_wider(names_from = burnsev, names_prefix = "burnsev_", values_from = total_ha:prop_grove_area)

severity_annual[is.na(severity_annual_wider)] <- 0
...
method_comparison_fire <- sti_mortality_est %>% 
  filter(aggregation == "fire", measure == "ct") %>% 
  as.data.frame()
...
ggplot(method_comparison_fire, aes(x = grove, y = value)) +
  ...
  theme_minimal()+
  xlab(NULL)+
  labs(title = "Comparison of mortality counts in groves based on STI counts and average trees per ha across\ngroves using fire aggregate mortality rates", y = "Tree mortality per grove")+
  theme(plot.title.position = "plot")
