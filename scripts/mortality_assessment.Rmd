...
sev_ba_all_sierra <- st_read("data/spatial_data/outputs/ba_all-sierra/ba_full_all.shp")

sev_ba_segi <- st_read("data/spatial_data/outputs/ba_cropped-groves/ba_segi_all.shp")

highsev_rdnbr <- rast("data/spatial_data/outputs/rdnbr_severe-fires_cropped/needFires_rdnbr.tif") %>% project(crs(sev_ba_all_sierra))
...
castle_meyer <- read_excel(here("data/plot_data/meyer/CastleFireGroveData_2023_combined.xlsx"), sheet = "Monarchs") %>% 
  ...
  select(survey_area, easting, northing, postfire_status, prefire_status, source, year, fire, gps_offset_m, gps_offset_deg)
...
meyers_corrected <- meyers_combined %>% 
  mutate(
    ...
    new_easting = easting + (sin(gps_reset_radians) * gps_offset_m), #use trig to offset x
