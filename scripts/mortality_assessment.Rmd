...
#four-class basal area severity for all fires that intersect groves, not clipped to groves
sev_ba_all_sierra <- st_read(here("data/spatial_data/outputs/ba_all-sierra/ba_full_all.shp"))

#four-class BA severity for fires that intersect groves, clipped to groves
...
#the sev_ba_all_sierra layer has all fires, but the observation data only are for recent fires, so remove duplicates
...
sti_sev <- sti %>% 
  st_join(sev_ba_all_sierra) %>%#here i include all times a tree experienced fire
  st_drop_geometry() %>% 
