...
sti <- st_read(here("data/spatial_data/inputs/forest/SEKI_STI_20211021.gdb"),layer = "SEKI_STI_pt") %>% 
  ...
  mutate(case_when(
    ...
    grove == "Devils Canyon" ~ "Devil's Canyon",
    T ~ grove
  )
  )

sti_sev <- sti %>% 
  st_join(sev_ba_all_sierra) %>%  # here i am leaving the multiple fires
  st_drop_geometry() %>% 
  drop_na(burnsev) %>%  # drop trees that did not experience fire
  summarise(.by = c(grove, burnsev), tree_ct = n())

grove_sev_area <- sev_ba_segi %>% 
  filter(grov_nm %in% sti_sev$grove) %>% 
  st_drop_geometry() %>% 
  summarise(.by = c(grov_nm, burnsev), area_ha = sum(area_ha))

grove_area <- st_read(here("data/spatial_data/inputs/forest/2023_GS_Groves_OTI_public.gdb"), layer = "OGS_Groves_2023_97_public") %>%
  clean_names() %>%
  st_transform(crs(sev_ba_segi)) %>% #uniform crs
  st_cast("MULTIPOLYGON") %>% #convert multisurface to multipolygon
  filter(grove_name %in% sti_sev$grove) %>% 
  st_drop_geometry() %>% 
  select(grove_name, acres) %>% 
  mutate(grove_ha = acres*0.404686) %>% select(!acres) %>% 
  left_join(grove_sev_area, join_by(grove_name == grov_nm))

sti_mortality_est <- sti_sev %>% 
  left_join(mortality_fire) %>% 
  left_join(mortality_year) %>% 
  left_join(grove_area_sti, join_by(grove == grov_nm, burnsev == burnsev))



  mutate(sti_min_fire = round(tree_ct*min_fire, 1),
        sti_min_perha_fire = mort_min_fire/,
         sti_max_fire = round(tree_ct*max_fire, 1),
         sti_max_perc_fire = proportion_groves*max_fire,
         sti_avg_fire = round(tree_ct*avg_fire, 1),
         sti_avg_perc_fire = proportion_groves*avg_fire,
         sti_min_yr = round(tree_ct*min_yr, 1),
         sti_min_perc_yr = proportion_groves*min_yr,
         sti_max_yr = round(tree_ct*max_yr, 1),
         sti_max_perc_yr = proportion_groves*max_yr,
         sti_avg_yr = round(tree_ct*avg_yr, 1),
         sti_avg_perc_yr = proportion_groves*avg_yr)
