...
Table with final column of trees per class to gut check
how many fires +trees per fire and measure delay
...
monarchs_severity <- monarchs_severity %>% 
  ...
  filter(!id.x %in% monarch_severity_duplicated$id.x, # remove all duplicated IDs
         measure_delay < 5, # swap to 5
         id.y %in% observation_counts$id.y
         ) %>% 
    rename("tree_id" = "id.x",
         "fire_id" = "id.y") %>% 
...
st_write(monarchs_severity, here("data/spatial_data/outputs/monarchs_severity/monarchs_severity_27Nov24a.shp"), append = F)
