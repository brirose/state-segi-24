...
swap workflow --> make sf before combining
...
summary_usgs <- st_drop_geometry(mort_usgs) %>% 
...
st_transform(crs(cbi_all)) %>% 
  ...
  select(id, grove, survey_year, postfire_status, prefire_status, source, survey_fire, dbh_cm) %>% 
  st_transform(crs(cbi_all))
...
#st_write(monarchs_combined, here("data/spatial_data/outputs/mortality_monarchs/mortality_monarchs_12Dec24.shp"), append = F)
#write_csv(monarchs_combined, here("data/outputs_data/mortality_monarchs.csv"))
