...
fire.pre <- st_read(here("data/spatial_data/inputs/fires_needed_rdnbr/cbi_all_r_ks.shp")) %>% 
...
fire.pre1 <- st_intersection(fire.pre, groves) %>% 
  st_cast("MULTIPOLYGON") %>% 
  st_cast("POLYGON")

# combine where there is buffer overlap and poorly drawn geometries
fire.pre2 <- fire.pre1 %>% 
  summarise(.by = c(year, burnsev), 
...
#reduce disturbance to within  groves
trt.pre1 <- st_intersection(trt.pre, groves) %>% 
  st_cast("MULTIPOLYGON") %>% 
  st_cast("POLYGON")
...
all_dist_order_cln = all_dist_order %>%
  filter(!resist_id %in% c(2285,4592))
...
##unclear if this really needs to be repaired? none were deleted with done in arcpro
# write_sf(dist_order_wide_summary_3trts, here("data/spatial_data/outputs/dist_order_wide_summary_3trts_raw_data.shp"))
# dist_order_wide_summary_3trts = write_sf(dist_order_wide_summary_3trts, here("data/spatial_data/outputs/dist_order_wide_summary_3trts_raw_data.shp"))
...
resist_polys_all = snapshot.resist %>%
  rbind(no_resist)
sum(resist_polys_all$area_ha)

resist_polys_summarized = resist_polys_all %>%
  group_by(resist_class) %>%
  summarise(area_ha = sum(area_ha))
sum(resist_polys_all$area_ha)
...
resist_polys_summarized_tbl = st_drop_geometry(resist_polys_summarized)
